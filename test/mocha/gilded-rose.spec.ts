...
describe("Updated Items", () => {
  describe("Updated Properties", () => {
    it("Can add ConjuredItems with the 'conjured' property set to true", () => {
      const gildedRose = new GildedRose([
        new UpdatedItem({ name: "conjured sword", sellIn: 10, quality: 10, conjured: true }),
      ]);
      expect(gildedRose.items[0]).to.haveOwnProperty("conjured", true);
    });
    it("'false' conjured property now included on traditional Items", () => {
      const gildedRose = new GildedRose([new Item("regular sword", 10, 10)]);
      expect(gildedRose.items[0]).to.haveOwnProperty("conjured", false);
    });
  });
  describe("Conjured Item has double Degradation", () => {
    it("degrades at -2 whilst SellIn >= 0", () => {
      const gildedRose = new GildedRose([
        new UpdatedItem({ name: "conjured foo", sellIn: 10, quality: 10, conjured: true }),
      ]);
      let items: ItemProps[];
      for (let i = 8; i >= 0; i -= 2) {
        items = gildedRose.updateQuality();
    ...
    it("degrades at -4 whilst SellIn < 0", () => {
      const gildedRose = new GildedRose([
        new UpdatedItem({ name: "conjured foo", sellIn: 0, quality: 20, conjured: true }),
      ]);
      let items: ItemProps[];
